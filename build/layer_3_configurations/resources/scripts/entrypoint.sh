...
if [[ ! -f "/opt/vars.lock" ]]; then
    ...
    if $INITIAL_SETUP_ENABLE; then
        TEMPLATE="/opt/liferay/templates/portal-setup-wizard.properties.template"
    ...
    if $PORTAL_EXT_SETUP_ENABLE; then
        TEMPLATE="/opt/liferay/templates/portal-ext.properties.template"
    ...
    if $REMOTE_DB_ENABLE; then    
        ...
        TEMPLATE="/opt/liferay/templates/server.xml.template"
        ...
        TEMPLATE="/opt/liferay/templates/context.xml.template"
    ...
    if $REMOTE_ES_ENABLE; then
        TEMPLATE="/opt/liferay/templates/com.liferay.portal.search.elasticsearch.configuration.ElasticsearchConfiguration.config.template"
